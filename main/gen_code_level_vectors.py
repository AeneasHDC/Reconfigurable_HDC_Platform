...
def generate_hls_code(file_name, DI_PARALLEL_W_BITS):
    ...
    hls_code.append("#ifndef LEVEL_VEC_GEN_H")
    hls_code.append("#define LEVEL_VEC_GEN_H")
    ...
    frame_index_width = math.ceil(math.log2(len(data_rows[0]) // DI_PARALLEL_W_BITS))
    frame_id_width = math.ceil(math.log2(len(data_rows)))

    hls_code.append(f"static void level_vec_gen(ap_uint<{DI_PARALLEL_W_BITS}> &level_vec_out, ap_uint<{frame_index_width}> frame_index, ap_uint<{frame_id_width}> frame_id) {{")
    hls_code.append("    // Update this function to match your requirements")
    hls_code.append("    switch(frame_index) {")

    for frame_index in range(0, len(data_rows[0]) // DI_PARALLEL_W_BITS):
        start_index = frame_index * DI_PARALLEL_W_BITS
        end_index = start_index + DI_PARALLEL_W_BITS
        hls_code.append(f"        case {frame_index}:")
        hls_code.append(f"            switch(frame_id) {{")
        for i, bits in enumerate(data_rows):
            if len(bits) >= end_index:
                hls_code.append(f"                case {i}:")
                hls_code.append(f"                    level_vec_out = 0b{bits[start_index : end_index]};")
                hls_code.append("                    break;")
            else:
                print(f"Skipping frame_id {i}, frame_index {frame_index} due to insufficient data. Length: {len(bits)}")
        hls_code.append("            }")
        hls_code.append("            break;")
...
def generate_sv_code(file_name, DI_PARALLEL_W_BITS):
    ...
    frame_index_bits = math.ceil(math.log2(n_cols // DI_PARALLEL_W_BITS))
    frame_id_bits = math.ceil(math.log2(n_rows))
    
    sv_code.append("module level_hvec_gen #(parameter int DI_PARALLEL_W_BITS = {}) (".format(DI_PARALLEL_W_BITS))
    ...
    sv_code.append("  output logic [DI_PARALLEL_W_BITS-1:0] level_vec_out")
    sv_code.append(");")
    ...
    for i in range(0, n_cols, DI_PARALLEL_W_BITS):
        ...
        for j in range(n_rows):
            ...
            sv_code.append("            level_vec_out = {}'b{};".format(DI_PARALLEL_W_BITS, row_bin))
...
def generate_verilog_code(file_name, DI_PARALLEL_W_BITS):
    ...
    verilog_code.append(f"module level_vec_gen (")
    verilog_code.append(f"    output reg [{DI_PARALLEL_W_BITS-1}:0] level_vec_out,")
    ...
    for frame_index in range(0, len(data_rows[0]) // DI_PARALLEL_W_BITS):
        start_index = frame_index * DI_PARALLEL_W_BITS
        end_index = start_index + DI_PARALLEL_W_BITS
        verilog_code.append(f"            {frame_index}: begin")
        verilog_code.append(f"                case (frame_id)")
        for i, bits in enumerate(data_rows):
            if len(bits) >= end_index:
                verilog_code.append(f"                    {i}: level_vec_out = {DI_PARALLEL_W_BITS}'b{bits[start_index : end_index]};")
            else:
                print(f"Skipping frame_id {i}, frame_index {frame_index} due to insufficient data. Length: {len(bits)}")
...
def generate_vhdl_code(file_name, DI_PARALLEL_W_BITS):
    ...
    vhdl_code.append(f"")
    vhdl_code.append(f"entity level_vec_gen is")
    vhdl_code.append(f"    Port ( level_vec_out : out  STD_LOGIC_VECTOR ({DI_PARALLEL_W_BITS-1} downto 0);")
    ...
    vhdl_code.append(f"end level_vec_gen;")
    vhdl_code.append(f"")
    vhdl_code.append(f"architecture Behavioral of level_vec_gen is")
    vhdl_code.append(f"begin")
    vhdl_code.append(f"    process (frame_index, frame_id)")
    vhdl_code.append(f"    begin")
    ...
    for frame_index in range(0, len(data_rows[0]) // DI_PARALLEL_W_BITS):
        start_index = frame_index * DI_PARALLEL_W_BITS
        end_index = start_index + DI_PARALLEL_W_BITS
        vhdl_code.append(f"            when \"{format(frame_index, f'0{frame_index_width}b')}\" =>")
        vhdl_code.append(f"                case frame_id is")
        for i, bits in enumerate(data_rows):
            if len(bits) >= end_index:
                vhdl_code.append(f"                    when \"{format(i, f'0{frame_id_width}b')}\" => level_vec_out <= \"{bits[start_index : end_index]}\";")
            else:
                print(f"Skipping frame_id {i}, frame_index {frame_index} due to insufficient data. Length: {len(bits)}")
...
FILE_NAME = relative_path_prefix + "out//extracted_params//"+config.DS_NAME+"//LevelHVs.csv"
# Read the CSV file
...
with open(OUTPUT_PATH+"level_hvec_gen_hdl.hpp", "w") as f:
    f.write(generate_hls_code(FILE_NAME, config.LV_IN_DATA_W_BITS))
# Write the Verilog code to a separate file
with open(OUTPUT_PATH+"level_hvec_gen_hdl.v", "w") as f:
    f.write(generate_verilog_code(FILE_NAME, config.LV_IN_DATA_W_BITS))
    # Write the VHDL code to a separate file
with open(OUTPUT_PATH+"level_hvec_gen_hdl.vhdl", "w") as f:
    f.write(generate_vhdl_code(FILE_NAME, config.LV_IN_DATA_W_BITS))
# Write the SystemVerilog code to a separate file
with open(OUTPUT_PATH+"level_hvec_gen_hdl.sv", "w") as f:
    f.write(generate_sv_code(FILE_NAME, config.LV_IN_DATA_W_BITS))